...
export const MenuCategoryItem: React.FC<MenuCategoryItemProps> = ({
    ...
    // Функция для активации первой подкатегории, если категория была активирована
    const handleCategoryWithFirstSubcategoryClick = () => {
        handleCategoryClick(categoryIndex);
        // Если категория активируется и в ней есть подкатегории, то активируем первую подкатегорию
        if (item.text.length > 0 && activeCategoryIndex !== categoryIndex) {
            handleSubcategoryClick(categoryIndex, 0);
        }
    };
    ...
    return (
        <AccordionItem border='none' key={categoryIndex}>
            <AccordionButton
                ...
                alignItems='center'
                pr={0}
                onClick={handleCategoryWithFirstSubcategoryClick} // Заменяем на новый обработчик
